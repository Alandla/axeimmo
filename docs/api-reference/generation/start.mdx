...
- **Base cost**: 30 credits for a 30-second video
- **Avatar usage**: +20 credits
- **Image animation**: +10 credits per image (standard), +25 credits per image (pro mode)
- **Web search**: +5 credits
- **Script generation**: Included in base cost

<Tip>
Use the `/script/generate` endpoint first to get an accurate cost estimate before generating the full video.
</Tip>
