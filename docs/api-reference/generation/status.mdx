...
api: "GET https://api.hoox.video/v1/generation/status/{jobId}"
...
```bash cURL
curl -X GET "https://api.hoox.video/v1/generation/status/run_abcd1234" \
...
job_id = "run_abcd1234"
url = f"https://api.hoox.video/v1/generation/status/{job_id}"
headers = {
...
const response = await fetch(`https://api.hoox.video/v1/generation/status/${jobId}`, {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer your_api_key'
  }
...
```json Processing
{
  ...
  "status": "processing",
  "progress_step": 45,
  "current_step": "Generating voice audio"
}
```
...
```json Failed
{
  ...
  "status": "failed",
  "progress_step": 25,
  "current_step": "Voice generation failed",
  "error": {
...
- **"Initializing generation"**: Setting up the job
- **"Generating script"**: Creating script from prompt
- **"Processing voice"**: Generating or processing voice audio
- **"Processing avatar"**: Preparing avatar for video
- **"Searching for media"**: Finding relevant images/videos
- **"Animating images"**: Applying AI animation to static images
- **"Assembling video"**: Combining all elements into final video
- **"Post-processing"**: Final video optimization and thumbnail generation
...
3. Monitor the export progress with `/export/status/{jobId}`