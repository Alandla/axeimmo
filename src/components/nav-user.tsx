          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <Avatar className="h-8 w-8 rounded-lg">
                {user?.image && <AvatarImage src={user.image} alt={(user.firstName || user.name) ?? ''} />}
                <AvatarFallback className="rounded-lg">
                  {loading ? '' : ((user.firstName || user.name)?.charAt(0).toUpperCase() ?? (user.email?.charAt(0).toUpperCase() ?? ''))}
                </AvatarFallback>
              </Avatar>
              <div className="grid flex-1 text-left text-sm leading-tight">
                {loading ? (
                  ...
                  <>
                    <span className="truncate font-semibold">{user.firstName || user.name}</span>
