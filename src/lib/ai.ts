...
export const analyzeDOMForInteraction = async (domContent: string): Promise<{ xpath: string; needCookies: boolean } | null> => {
  try {
    const groq = new Groq({
      apiKey: process.env.GROQ_API_KEY!,
    });

    const prompt = `You are helping the user automate the browser by finding elements based on what the user wants to observe in the page. Your task is to analyze the provided DOM content and identify interactive elements for two specific purposes.

**Priority Order:**
1. FIRST: Check if there's a cookie consent banner that needs to be accepted
2. SECOND: If no cookie banner, look for buttons to expand/view more images

**Cookie Banner Detection:**
Look for elements that indicate cookie consent banners:
- Text containing: "accept", "cookies", "consent", "agree", "privacy", "terms"
- Common class names: "cookie", "consent", "privacy", "banner", "notice"
- Buttons or links for accepting cookies
- Modal dialogs or overlays about cookies/privacy

**Image Expansion Detection:**
Find buttons or elements that might reveal additional photos:
- Text like: "view all photos", "show more images", "see all pictures", "more photos", "load more", "view gallery", "expand images"
- Buttons with image-related icons or text
- Gallery expansion controls
- "Show more" or "Load more" buttons in image contexts
- French equivalents: "voir plus de photos", "afficher plus", "voir toutes les photos", "charger plus"

**Instructions:**
1. If you find a cookie banner, return its XPath with needCookies: true
2. If no cookie banner but you find image expansion elements, return the XPath with needCookies: false  
3. If neither exists, return an empty xpath with needCookies: false

**CRITICAL XPath Requirements:**
- You MUST provide FULL ABSOLUTE XPath expressions that start with /html/body/
- Example format: /html/body/div[1]/div/main/div/div[2]/div[1]/div[2]/div[3]/div/div
- DO NOT use relative XPath like //button or //div
- DO NOT use XPath functions like contains(), text(), or @class
- Count the exact position of each element in the DOM hierarchy
- Provide the complete path from html root to the target element

**Response Format:**
Return ONLY a valid JSON object with this exact structure:
{
  "xpath": "the full absolute xpath or empty string if nothing found",
  "needCookies": true or false
}

**DOM Content to analyze:**
${domContent}

Analyze the DOM carefully and provide the appropriate response with a full absolute XPath:`;
    ...
    if (typeof jsonResponse.xpath !== 'string' || typeof jsonResponse.needCookies !== 'boolean') {
      console.error('Invalid response structure from DOM analysis');
      return null;
    }

    // Validate that XPath is absolute if provided
    if (jsonResponse.xpath && !jsonResponse.xpath.startsWith('/html/body/')) {
      console.error('XPath is not absolute, received:', jsonResponse.xpath);
      return null;
    }

    return jsonResponse;
  } catch (error) {
    console.error('Error analyzing DOM for interaction:', error);
    return null;
  }
};